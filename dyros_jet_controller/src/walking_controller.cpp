...
void WalkingController::getCapturePointTrajectory()
{
  ...
  if(walking_tick_==0)
  {
    previewControlParameter(1.0/hz_, 16*hz_/10, k_ ,com_support_init_, gi_, gp_l_, gx_, a_, b_, c_);
      ...
      capturePoint_offset_(1) = 0.02; //CapturePointy offset
        ...
        for(int i = 1; i<total_step_num_+2; i++)
        {
          zmp_dx(i) = capturePoint_ox(i+1)/(1-b)-(b*capturePoint_ox(i))/(1-b);
          zmp_dy(i) = capturePoint_oy(i+1)/(1-b)-(b*capturePoint_oy(i))/(1-b);
        }
          zmp_dx(0) = capturePoint_ox(1)/(1-b_init)-(b*capturePoint_ox(0))/(1-b_init);
          zmp_dy(0) = capturePoint_oy(1)/(1-b_init)-(b*capturePoint_oy(0))/(1-b_init);
      ...
      for(int i=0; i<(t_total_*(total_step_num_+1)+t_temp_+1); i++)
      {
        ...
        if(i<t_temp_+t_total_+t_rest_init_+t_double1_-1)
        {
          ...
          {
            tick = (i-(t_temp_+t_rest_init_+t_double1_-1))/(hz_);
          }
        ...
        //ZMP trajectory from CP
         if(current_step == 0)
         {
           ...
           else if(1.5*hz_ <= i && i <= t_temp_+t_rest_init_)
           {
             zmp_refx(i) = com_support_init_(0)+(zmp_dx(0)-com_support_init_(0))*(i+1-1.5*hz_)/(t_temp_+t_rest_init_+1-1.5*hz_);
             zmp_refy(i) = com_float_init_(1)+(zmp_dy(0)-com_float_init_(1))*(i+1-1.5*hz_)/(t_temp_+t_rest_init_+1-1.5*hz_);
           }
           else
           {
             kx = zmp_dx(1)-zmp_dx(0);
             ky = zmp_dy(1)-zmp_dy(0);
             ...
             {
               zmp_refx(i) = kx*(i+1-t_rest_init_-t_temp_)/t_double1_+zmp_dx(0);
               zmp_refy(i) = ky*(i+1-t_rest_init_-t_temp_ )/t_double1_+zmp_dy(0);
             }
        ...
        if(capturePointChange==0 && i<1.5*hz_)
        {
          capturePoint_refx(i) = capturePoint_ox(0);
          capturePoint_refx(i) = capturePoint_oy(0);
        }
        else if(capturePointChange==0 && 1.5*hz_ <= i && i <= t_temp_+t_rest_init_)
        {
          tick = (i-1.5*hz_)/hz_;
          capturePoint_refx(i) = exp(w*tick)*capturePoint_ox(capturePointChange)+(1-exp(w*tick))*capturePoint_ox(0);
          capturePoint_refy(i) = exp(w*tick)*capturePoint_oy(capturePointChange)+(1-exp(w*tick))*capturePoint_oy(0);
        }
        else if(capturePointChange==0 &&  t_rest_init_+t_temp_ <= i && i < t_rest_init_+t_double1_+t_temp_)
        {
          tick = (i-t_rest_init_-t_temp_)/hz_;
          capturePoint_refx(i) = exp(w*tick)*capturePoint_ox(capturePointChange)+(1-exp(w*tick))*zmp_dx(capturePointChange);
          capturePoint_refy(i) = exp(w*tick)*capturePoint_oy(capturePointChange)+(1-exp(w*tick))*zmp_dy(capturePointChange);
        }
        else if(capturePointChange==0 &&  i >= t_rest_init_+t_double1_+t_temp_)
        {
          capturePoint_refx(i) = exp(w*tick)*capturePoint_ox(capturePointChange)+(1-exp(w*tick))*zmp_dx(capturePointChange);
          capturePoint_refy(i) = exp(w*tick)*capturePoint_oy(capturePointChange)+(1-exp(w*tick))*zmp_dy(capturePointChange);
        }
 ...
 previewControl_cap(1.0/hz_, 16*hz_/10, walking_tick_, xi, yi, xs, ys, ux_1_, uy_1_, ux_, uy_, gi_, gp_l_, gx_, a_, b_, c_, xd, yd);
  ...
  xs = xd;
  ys = yd;
  file[14]<<capturePoint_refx(walking_tick_)<<"\t"<<zmp_refx(walking_tick_)<<"\t"<<xyd2(1)<<"\t"<<yd(0)<<"\t"<<current_step_float_support_(1,3)<<"\t"<< current_step_support_float_(1,3)<<endl;
