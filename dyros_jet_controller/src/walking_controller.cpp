...
void WalkingController::usingFootStepPlanner()
{
  ...
  int step_size_1 = 0;
  double temp_yaw, temp_yaw1;
  for(int i = 0; i<9; i++)
  {
    if(datafoot[step_size_glo+foot_step_cycle+1]==20 || datafoot[step_size_glo+foot_step_cycle+1]==100)
    {
     /* if(first_step == false)
      {
        step_size_glo += 1;
  ...
  /*
0 -0.059889 0.110987 0.459984
2 0.069914 -0.101250 0.649328

   * 0 -0.100554 0.080158 0.848856
2 0.100848 -0.065398 1.050033
0 -0.127158 0.034557 1.258302
2 0.115733 -0.016463 1.473888*/

  std::cout << "step_size_glo " <<step_size_glo << "step_size_1" << step_size_1 << std::endl;
  foot_step_plan_num_ = step_size_1;
  ...
  if(first_step == false)
  {
    temp = datafoot[step_size_glo-step_size_1]-1 ;
  }
  else
  {
    temp = datafoot[step_size_glo-step_size_1+foot_step_cycle]-1 ;
  }

  int index = 0;
  for (int i =0 ; i<foot_step_plan_num_; i++)
  {
    if(datafoot[step_size_glo-step_size_1+foot_step_cycle]!=20)
    {
      if(i==0)
      {
        temp_yaw = 0;
        temp_yaw1 = dataYaw[step_size_glo-step_size_1+foot_step_cycle+i];
      }
      else
      {
        temp_yaw = temp_yaw1;
    ...
    else
    {
      ...
      {
        temp_yaw = 0;
        temp_yaw1 = dataYaw[step_size_glo-step_size_1+i+foot_step_cycle];
      }
      else
      {
        temp_yaw = temp_yaw1;
    ...
    if(datafoot[step_size_glo-step_size_1+foot_step_cycle]!=20)
    {
      foot_step_(index,0) = datax[step_size_glo-step_size_1+i+foot_step_cycle];
      foot_step_(index,1) = datay[step_size_glo-step_size_1+i+foot_step_cycle];
      foot_step_(index,5) = dataYaw[step_size_glo-step_size_1+i+foot_step_cycle]-temp_yaw;
      foot_step_(index,6) = 0.5+0.5*temp;
    }
    else
    {
    ...
    std::cout << "index" << i <<"x "<< foot_step_(index,0) <<"yaw " <<foot_step_(index,1) <<"yaw " <<foot_step_(index,5)<<"yaw " <<foot_step_(index,6)<<std::endl;
...
void WalkingController::updateNextStepTime()
{
 ...
 if(current_step_num_ == total_step_num_-1 && walking_tick_ >= t_last_ +3.0*hz_+1)
 {
   if(datafoot[step_size_glo+foot_step_cycle] == 20 && datafoot[step_size_glo+foot_step_cycle+1]!=100)
   {
     foot_step_cycle +=1;
...
void WalkingController::compensator()
{
  ...
  if(lqr_compensator_mode_ == true)
  {
    ...
    for (int n = 7 ; n < 12; n++) // left foot
    {
      ...
      {
        if(n == 7 || n == 8)
          desired_q_(n) = desired_q_(n) - 0.0021*grav_gain_timing*grav_ground_torque_[n];
        else if (n == 9)
