...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  valid_vars = 0;
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "Ai");
  if (xm == NULL) {
    printf("could not find params.Ai.\n");
  } else {
    if (!((mxGetM(xm) == 8) && (mxGetN(xm) == 6))) {
      printf("Ai must be size (8,6), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_vamexFunctionr_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter Ai must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter Ai must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter Ai must be a full matrix.\n");
      this_var_errors++;
    }
  ...
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "bi");
  if (xm == NULL) {
    printf("could not find params.bi.\n");
  } else {
    if (!((mxGetM(xm) == 8) && (mxGetN(xm) == 1))) {
      printf("bi must be size (8,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter bi must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter bi must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter bi must be a full matrix.\n");
      this_var_errors++;
    }
    if (this_var_errors == 0) {
      dest = params.bi;
  ...
  if (prepare_for_c) {
    ...
    for (i = 0; i < 48; i++)
      printf("  params.Ai[%d] = %.6g;\n", i, params.Ai[i]);
    for (i = 0; i < 8; i++)
      printf("  params.bi[%d] = %.6g;\n", i, params.bi[i]);
  }
