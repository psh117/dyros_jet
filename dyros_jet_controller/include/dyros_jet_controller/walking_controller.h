...
public:
    ...
    total_dof_(DyrosJetModel::HW_TOTAL_DOF), model_(model), current_q_(current_q), hz_(hz), current_time_(control_time), start_time_{}, end_time_{}, slowcalc_thread_(&WalkingController::slowCalc, this), calc_update_flag_(false), calc_start_flag_(false), ready_for_thread_flag_(false), ready_for_compute_flag_(false), foot_step_planner_mode_(false), walking_end_foot_side_ (false), foot_plan_walking_last_(false)
  {
    ...
    first_step = false;
     foot_step_cycle = 0;
    ifstream footdata("/home/junhyung/data1.txt");
    if(footdata.is_open())
    {
      int tempfoot1; double tempx1, tempy1, tempYaw1;
      char c;
       ...
       while(getline(footdata,line))
       {
         ...
         ss >> buff>> buff1 >> buff2 >>buff3;

         if (buff != "end")
         {
           tempfoot1 = std::stoi(buff);
           ss1 << buff1;
           ss1 >> dou;
           tempx1 = dou;
           ss2 << buff2;
           ...
           ss3 >> dou2;
           tempYaw1 =dou2;
        /*   tempx1 = std::atof(buff[1].c_str());
           tempy1 = std::atof(buff[2].c_str());
           tempYaw1 = std::atof(buff[3].c_str());*/
           datafoot.push_back(tempfoot1);
           datax.push_back(tempx1);
           datay.push_back(tempy1);
           dataYaw.push_back(tempYaw1);
         }
         else if (buff == "end")
         {
           tempfoot1 = 20;
    ...
    for(int i = 0; i<30; i++)
    {
      std::cout << "footdata" << datafoot[i] << "dsss  " << datay[i]<< std::endl;
    }
...
private:
  ...
  std::vector<int> datafoot;
  std::vector<double> datax;
  std::vector<double> datay;
  std::vector<double> dataYaw;
